...
public class CustomerService {
    ...
    public void uploadCustomerProfileImage(Integer customerID, MultipartFile file) {
        ...
        try {
            s3Service.putObject(
                s3Buckets.getCustomer(), "profile-images/%s/%s".formatted(customerID, profileImageID), file.getBytes()
            );
        } catch (IOException e) {
    ...
    public byte[] getCustomerProfileImage(Integer customerID) {
        ...
        if (customer.profileImageId().isBlank()) {
            throw new ResourceNotFoundException("customer with id profile image [%s] not found".formatted(customerID));
        }
